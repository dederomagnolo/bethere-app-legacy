...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const updatePumpFromRemote = async () => {
      try {
        let remainingTime;
        const lastStatusAll = await api.post(
          `${bethereUrl}/commands/laststatus/all`
        );
        const lastAutoPumpStatus = _.get(
          lastStatusAll,
          "data.autoPump.commandName"
        );
        const lastPumpStatus = _.get(
          lastStatusAll,
          "data.manualPump.commandName"
        );
        ...
        if (
          remainingTime &&
          (remainingTime.mins < 0 || remainingTime.secs < 0)
        ) {
          setPumpFlag(false);
        } else {
    ...
    const updateFeedFromRemote = async () => {
      try {
        ...
        const measuresFromRemote = {
          internalHumidity:
            internalHumidity && internalHumidity !== "nan"
              ? Number(internalHumidity).toFixed(2)
              : "-",
          internalTemperature:
            internalTemperature && internalTemperature !== "nan"
              ? Number(internalTemperature).toFixed(2)
              : "-",
          externalHumidity:
            externalHumidity && externalHumidity !== "nan"
              ? Number(externalHumidity).toFixed(2)
              : "-",
          externalTemperature:
            externalTemperature && externalTemperature !== "nan"
              ? Number(externalTemperature).toFixed(2)
              : "-",
        };
    ...
    const updateFields = async (fieldNumber) => {
      ...
      const queryStart = `${moment(selectedDate).format(
        "YYYY-MM-DD"
      )}%2003:00:00`;
      ...
      try {
        const response = await api.get(
          `${thingspeakUrl}/fields/${fieldNumber}.json?start=${queryStart}&end=${queryEnd}`
        ); //
        ...
        await _.each(weekFeed, (entry, index) => {
          ...
          data.push({
            ...
            y:
              fieldMeasure && fieldMeasure !== "nan"
                ? Number(fieldMeasure).toFixed(2)
                : "30",
          });
          // }
        });
    ...
    const getLocalStationStatus = async () => {
      try {
        setLoading(true);
        const localStationStatus = await api.post(`${bethereUrl}/ls-status`, {
          deviceSerialKey,
        });
        setLocalStationStatus(
          _.get(localStationStatus, "data.isDeviceConnected")
        );
        setLoading(false);
      } catch (err) {
        setLoading(false);
    ...
    if (localStationStatus) {
      return !pumpFlag ? (
        <span style={{ color: "green" }}>
          {translate("localStationStatusOnLabel")}
        </span>
      ) : (
    ...
    <MainContainer>
      ...
      <div>
          ...
          <NewCard
            ...
            children={
              loading ? (
                ...
                >
                  <div>Auto: {renderAutoWateringLabel()}</div>
        ...
        {showTemperatureChart && temperatureData.length > 0 && (
          <Graph chartData={temperatureData} />
        )}
        {showHumidityChart && humidityData.length > 0 && (
