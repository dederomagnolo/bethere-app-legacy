...
const History = () => {
  ...
  const [deviceSettings, setDeviceSettings] = useState(
    _.get(userDevices, "[0].settings[0]")
  );
  const [selectedDevice, setSelectedDevice] = useState(
    _.get(userDevices, "[0]")
  );
  const [selectedDate, setSelectedDate] = useState(
    moment().tz("America/Sao_Paulo").startOf("day").format()
  );
  ...
  const handleGetHistory = async (date) => {
    const res = await callApi("POST", "/commands/history", {
      deviceId,
        ...
        {_.map(orderedCommandsList, (command) => {
          ...
          return (
            <CommandCard key={createdAt}>
              ...
              <CommandLabels>
                <CommandCategory>
                  {getCategoryLabel(commandCategory)}
                </CommandCategory>
